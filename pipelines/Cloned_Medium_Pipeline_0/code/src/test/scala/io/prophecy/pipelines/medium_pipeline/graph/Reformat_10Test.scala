...
class Reformat_10Test extends FunSuite with DataFrameSuiteBase {
  ...
  test("Unit Test ") {
    ...
    val dfIn = createDfFromResourceFiles(
      spark,
      "/data/io/prophecy/pipelines/medium_pipeline/graph/Reformat_10/in/schema.json",
      "/data/io/prophecy/pipelines/medium_pipeline/graph/Reformat_10/in/data/unit_test_.json",
      "in"
    )
    val dfOut = createDfFromResourceFiles(
      spark,
      "/data/io/prophecy/pipelines/medium_pipeline/graph/Reformat_10/out/schema.json",
      "/data/io/prophecy/pipelines/medium_pipeline/graph/Reformat_10/out/data/unit_test_.json",
      "out"
    )

    val dfOutComputed =
      io.prophecy.pipelines.medium_pipeline.graph.Reformat_10(context, dfIn)
