...
class reformatted_columnsTest extends FunSuite with DataFrameSuiteBase {
  ...
  test("Unit Test ") {
    ...
    val dfIn = createDfFromResourceFiles(
      spark,
      "/data/io/prophecy/pipelines/medium_pipeline/graph/reformatted_columns/in/schema.json",
      "/data/io/prophecy/pipelines/medium_pipeline/graph/reformatted_columns/in/data/unit_test_.json",
      "in"
    )
    val dfOut = createDfFromResourceFiles(
      spark,
      "/data/io/prophecy/pipelines/medium_pipeline/graph/reformatted_columns/out/schema.json",
      "/data/io/prophecy/pipelines/medium_pipeline/graph/reformatted_columns/out/data/unit_test_.json",
      "out"
    )

    val dfOutComputed = io.prophecy.pipelines.medium_pipeline.graph
      .reformatted_columns(context, dfIn)
    val res = assertDFEquals(
      dfOut.select(
        ...
        "c  date",
        "c_timestamp",
        "c_expression_contains_startswith",
        "c_expression_contains_startswith_1"
      ),
