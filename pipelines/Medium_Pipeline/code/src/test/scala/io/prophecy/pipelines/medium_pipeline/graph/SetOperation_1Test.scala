...
class SetOperation_1Test extends FunSuite with DataFrameSuiteBase {
  ...
  test("Unit Test ") {
    ...
    val dfIn0 = createDfFromResourceFiles(
      spark,
      "/data/io/prophecy/pipelines/medium_pipeline/graph/SetOperation_1/in0/schema.json",
      "/data/io/prophecy/pipelines/medium_pipeline/graph/SetOperation_1/in0/data/unit_test_.json",
      "in0"
    )
    val dfIn1 = createDfFromResourceFiles(
      spark,
      "/data/io/prophecy/pipelines/medium_pipeline/graph/SetOperation_1/in1/schema.json",
      "/data/io/prophecy/pipelines/medium_pipeline/graph/SetOperation_1/in1/data/unit_test_.json",
      "in1"
    )
    val dfOut = createDfFromResourceFiles(
      spark,
      "/data/io/prophecy/pipelines/medium_pipeline/graph/SetOperation_1/out/schema.json",
      "/data/io/prophecy/pipelines/medium_pipeline/graph/SetOperation_1/out/data/unit_test_.json",
      "out"
    )

    val dfOutComputed = io.prophecy.pipelines.medium_pipeline.graph
      .SetOperation_1(context, dfIn0, dfIn1)
    val res = assertDFEquals(dfOut.select("c_string_new"),
                             dfOutComputed.select("c_string_new"),
