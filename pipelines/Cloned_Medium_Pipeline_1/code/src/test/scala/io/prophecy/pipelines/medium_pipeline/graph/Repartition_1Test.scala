...
class Repartition_1Test extends FunSuite with DataFrameSuiteBase {
  ...
  test("Unit Test ") {
    ...
    val dfIn = createDfFromResourceFiles(
      spark,
      "/data/io/prophecy/pipelines/medium_pipeline/graph/Repartition_1/in/schema.json",
      "/data/io/prophecy/pipelines/medium_pipeline/graph/Repartition_1/in/data/unit_test_.json",
      "in"
    )
    val dfOut = createDfFromResourceFiles(
      spark,
      "/data/io/prophecy/pipelines/medium_pipeline/graph/Repartition_1/out/schema.json",
      "/data/io/prophecy/pipelines/medium_pipeline/graph/Repartition_1/out/data/unit_test_.json",
      "out"
    )

    val dfOutComputed =
      io.prophecy.pipelines.medium_pipeline.graph.Repartition_1(context, dfIn)
    val res = assertDFEquals(
      dfOut.select(
        "c_array_int",
        "c_array_string",
        "c_array_boolean_renamed",
        "c_array_date",
        "c_array_timestamp",
        "c_array_float",
        "c_array_decimal",
