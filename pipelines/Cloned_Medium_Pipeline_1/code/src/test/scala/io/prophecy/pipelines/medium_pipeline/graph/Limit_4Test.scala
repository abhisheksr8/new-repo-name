...
class Limit_4Test extends FunSuite with DataFrameSuiteBase {
  ...
  test("Unit Test ") {
    ...
    val dfIn = createDfFromResourceFiles(
      spark,
      "/data/io/prophecy/pipelines/medium_pipeline/graph/Limit_4/in/schema.json",
      "/data/io/prophecy/pipelines/medium_pipeline/graph/Limit_4/in/data/unit_test_.json",
      "in"
    )
    val dfOut = createDfFromResourceFiles(
      spark,
      "/data/io/prophecy/pipelines/medium_pipeline/graph/Limit_4/out/schema.json",
      "/data/io/prophecy/pipelines/medium_pipeline/graph/Limit_4/out/data/unit_test_.json",
      "out"
    )

    val dfOutComputed =
      io.prophecy.pipelines.medium_pipeline.graph.Limit_4(context, dfIn)
    val res = assertDFEquals(
      dfOut.select("c   short  --",         "c-int-column type", "c--boolean"),
      dfOutComputed.select("c   short  --", "c-int-column type", "c--boolean"),
